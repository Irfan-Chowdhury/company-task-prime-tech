    ...
    <div class="container p-4">
        ...
        <form action="{{ route('companies.update', $company->id) }}" method="POST">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <input type="text" name="name" value="{{ $company->name }}" class="form-control" placeholder="Ex: Prime Tech Solution">
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label">Address</label>
                <div class="col-sm-10">
                    <textarea name="address" name="address" cols="30" rows="5" class="form-control">{{ $company->address }}</textarea>
                </div>
            </div>
            ...
            <div class="form-group" id="item-list">
                    ...
                    @if ($company->customizeFields)
                        @foreach ($company->customizeFields as $key => $item)
                        <div id="{{ $key  }}" class="item">
                            <input type="hidden" class="customize_field_id" name="invoice_item_id[]" value="{{$item->id}}"/>

                            <div class="form-group row">
                                <label class="col-sm-2 col-form-label">Label Name</label>
                                <div class="col-sm-3">
                                    <input type="text" value="{{ $item->label }}" name="labels[]" class="form-control">
                                </div>
                                <label class="col-sm-2 col-form-label">Field Value</label>
                                <div class="col-sm-3">
                                    <input type="text" value="{{ $item->value }}" name="values[]" class="form-control">
                                </div>
                                <div class="col-sm-2">
